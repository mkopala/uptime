...
Monitor.prototype.pollCheck = function(check, callback) {
  ...
  var pollerCallback = function(err, time, res, pollerDetails) {
    // Update interval for check on Up/Down status change
    if (err || !check.isUp && res.statusCode == 200) {
      var options = url.parse(self.config.apiUrl + '/checks/' + check._id);
      options.method = 'POST';
      // Set the interval to 30s if site went down, and 10 minutes if back up
      var postData = 'interval=' + (err ? 30 : 600);
      options.headers = {
        'Content-Type': 'application/x-www-form-urlencoded',
        'Content-Length': Buffer.byteLength(postData)
      };
      self.applyApiHttpOptions(options);
